...
select p.PATID
      ,d.BIRTH_DATE
      ,round(DATE_PART('day', p.INDEX_DATE - d.BIRTH_DATE) / 365.25) AGE_AT_ENROLL
      ,d.SEX
      ,d.RACE
      ,d.HISPANIC
...
--      ,a.ADDRESS_ZIP9  -- not available
      ...
      ,round(DATE_PART('day', e.ADMIT_DATE - p.INDEX_DATE) ) PAYER_DAYS_SINCE_ENROLL
      ,e.ADMIT_DATE
...
where e.ADMIT_DATE <= p.INDEX_DATE -- so that the payor 
)
select PATID
      ,BIRTH_DATE
      ,AGE_AT_ENROLL
...
select distinct p.PATID
      ,COALESCE(l.SPECIMEN_DATE,l.LAB_ORDER_DATE) as LAB_DATE 
      ,'TC' as LAB_NAME
      ,l.RESULT_NUM
      ,round(DATE_PART('day', COALESCE(l.SPECIMEN_DATE,l.LAB_ORDER_DATE) - p.INDEX_DATE)) as LAB_DAYS_SINCE_ENROLL
from pat_incld p 
...
where ( 
       ...
       and l.SPECIMEN_SOURCE is distinct from  'URINE' /*only serum creatinine*/
...
select  dx.PATID
      ...
      ,round(DATE_PART('day', dx.DX_DATE - p.INDEX_DATE) ) as COND_DAYS_SINCE_ENROLL
from pat_incld p
...
select px.PATID
      ...
      ,round(DATE_PART('day', px.PX_DATE  - p.INDEX_DATE)) as COND_DAYS_SINCE_ENROLL
from pat_incld p
...
select lab.PATID
      ...
      ,'CKD' as CONDITION
      ,COALESCE(lab.SPECIMEN_DATE,lab.LAB_ORDER_DATE) as CONDITION_DATE 
      ,round(DATE_PART('day', COALESCE(lab.SPECIMEN_DATE,lab.LAB_ORDER_DATE) - p.INDEX_DATE)) as COND_DAYS_SINCE_ENROLL
from pat_incld p
...
select distinct
      ...
      ,round(DATE_PART('day', m.RX_START_DATE - p.INDEX_DATE)) as DAYS_SINCE_ENROLL
...
select distinct
      ...
      ,cast(null as date) as END_DATE --no end_date for dispensing data
      ,cs.DRUG_CLASS
      ...
      ,round(DATE_PART('day',d.DISPENSE_DATE - p.INDEX_DATE)) as DAYS_SINCE_ENROLL
...
select p.PATID
      ,oc.ENCOUNTERID
      ,'HT' as VITAL_TYPE
      ,oc.OBSCLIN_RESULT_text::integer as VITAL_VAL
...
select p.PATID
      ,v.ENCOUNTERID
      ,'BMI' as VITAL_TYPE
      ,coalesce(round(v.WT/(v.HT*v.HT)*703),v.ORIGINAL_BMI) as VITAL_VAL -- BMI = (Weight in Pounds / (Height in inches) x (Height in inches)) x 703
      ,'kg/m2'
      ,round(DATE_PART('day', v.MEASURE_DATE - p.INDEX_DATE)) as DAYS_SINCE_ENROLL
from pat_incld p
join VITAL v on v.PATID = p.PATID 
