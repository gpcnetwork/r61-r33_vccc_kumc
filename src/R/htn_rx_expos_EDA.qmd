...
# Specify a null model with no predictors
null_model <- polr(
  delta_sbp_over10_sign ~ 1,
  data = sel_aset
)

# Specify the full model using all of the potential predictors
full_model <- polr(
  formula(paste0("delta_sbp_over10_sign ~ ",paste(var_sel,collapse = "+"))), 
  data = sel_aset, 
  Hess = TRUE
)
...
data.frame(coefs) %>% mutate(pval = p)
```
...
enrol_aset<-baseline_aset %>% mutate(PRESCREEN_STATUS = "study") %>%
  ...
  mutate(
    ...
    BASE_SBP_GRP = case_when(
      BASE_SBP<140 ~ 'sbpgrp1',
      BASE_SBP>=140&BASE_SBP<150 ~ 'sbpgrp2',
      BASE_SBP>=150&BASE_SBP<160 ~ 'sbpgrp3',
      BASE_SBP>=160 ~ 'sbpgrp4',
      TRUE ~ NA_character_
    ),
  ...
  filter(!is.na(BASE_SBP)&!is.na(AGE_GRP))
