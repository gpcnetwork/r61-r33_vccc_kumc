...
kable(
  ...
  caption = "Table 2: Cohort Characteristics on CKD Testing)"
)
```
...
fit_uni <- map(var_pre, ~ glm(as.formula(paste("CKD_TEST_IND ~", .x)), 
...
full_model <- glm(
  formula(paste0("CKD_TEST_IND ~ ",paste(var_pre,collapse = "+"))), 
  data = aset_complt, 
  family = binomial()
)
